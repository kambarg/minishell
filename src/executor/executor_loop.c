...
/*   Updated: 2025/07/15 17:02:07 by wuabdull         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	*last_pid = -1;
	while (cmd)
	{
		if (cmd->arg_count > 0 && is_builtin(cmd->args[0].value)
			&& (!cmd->next || shell->commands == cmd))
			last_status = handle_builtin_cmd(cmd, shell,
...
int	wait_and_return_status(pid_t last_pid,
